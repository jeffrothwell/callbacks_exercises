...
// I did it as above, then Sean showed me this:
...
function toVendor(purchase){
  return purchase.vendor;
}

allVendors = transactions.filter(isPurchase).map(toVendor);
...
transactions.forEach(function(transaction){
  if (transaction.type === 'sale' && !uniqueCustomers.includes(transaction.customer)) {
...
//
// transactions.forEach(function(transaction){
//   if (transaction.type === 'sale' && sumSales === 0) {
...
var sumSales = transactions.filter(isSale).reduce(addTransaction, 0);
...
function sumItems(transaction){
  return transaction.items.reduce(function(total, item){
    return total + item.price;
  }, 0)
}

function addTransaction(total, transaction){
  return total + sumItems(transaction);
}

var sumPurchases = transactions.filter(isPurchase).reduce(addTransaction, 0);
...
sumSales = transactions.filter(isSale).reduce(addTransaction, 0);
...
transactions.forEach(function(transaction){
  if (transaction.type === 'sale' && transaction.items.length > mostItems) {
...
transactions.forEach(function(transaction){
  if (transaction.type === 'purchase' && !leastItems) {
    leastItems = transaction.items.length
  } else if (transaction.type === 'purchase' && transaction.items.length < leastItems) {
    leastItems = transaction.items.length
    sumOfSmallestPurchase = sumItems(transaction);
  }
});
